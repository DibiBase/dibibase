...
catalog::Record io::DiskManager::get_record_from_data(
  ...
  std::unique_ptr<unsigned char[]> s_buf =
      std::unique_ptr<unsigned char[]>(new unsigned char[record_prefix_size]);
  int rc = read(fd, s_buf.get(), record_prefix_size);

  if (rc < 0)
    util::Logger::make().err("Error reading Data file: %d", errno);

  util::Buffer *record_size_buffer =
      new util::MemoryBuffer(std::move(s_buf), record_prefix_size);
