...
Commitlog::Commitlog(const char *filename) : max_size(COMMITLOG_MAX_SIZE_IN_MEGABYTES * 1024 / 10) {
  ...
  if ((log_file_descriptor == -1) && (EEXIST == errno)) {
    ...
    if (read_file_header())
      std::cout << "File header read successfully - Stats: archived=" << is_archived() << " active=" << is_active()
                << std::endl;
...
bool Commitlog::read_file_header() {
  ...
  memcpy(reinterpret_cast<unsigned char *>(&append_position), buffer.get() + Commitlog::APPEND_POSITION_OFFSET,
         sizeof(int32_t));
  return true;
}
